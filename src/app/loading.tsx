...
export default function WeatherLoading() {
  return (
    <div className="relative w-screen min-h-screen md:h-screen flex justify-center items-center overflow-hidden bg-gradient-to-br from-blue-400 to-blue-600">
      {/* Blurred background placeholder */}
      <div className="absolute -inset-4 bg-gradient-to-br from-blue-500 to-purple-600 blur-lg opacity-50" />

      {/* Main content window */}
      <div className="relative z-10 w-11/12 md:h-4/5 bg-white/10 backdrop-blur-md rounded-xl shadow-lg text-white flex md:flex-row outline outline-8 outline-white/25 flex-col my-4 md:my-0">
        ...
        <article className="flex flex-col md:w-8/12 lg:w-9/12 h-full p-4 justify-between md:order-first">
          ...
          <section className="my-4 lg:my-0 space-y-6">
            ...
            <div className="text-end">
              <Skeleton className="h-12 md:h-16 w-3/4 ml-auto rounded-lg bg-white/20 mb-6" />
            </div>
            ...
            <div className="flex overflow-x-auto space-x-4 scrollbar-hidden">
              {Array.from({ length: 8 }).map((_, index) => (
                <div
                  key={index}
                  className="flex flex-col items-center space-y-3 min-w-[80px] p-3 bg-white/10 rounded-lg backdrop-blur-sm"
                >
        ...
        {/* Side Panel Skeleton */}
        <Card className="flex flex-col md:w-5/12 lg:w-3/12 items-center h-screen md:h-full bg-white/5" isBlurred>
          ...
          <div className="flex flex-col w-10/12 items-center my-4 md:my-0 lg:my-4 space-y-4">
            ...
            <div className="flex justify-between space-x-2">
              {Array.from({ length: 3 }).map((_, index) => (
                <Skeleton key={index} className="h-8 w-16 rounded bg-white/20" />
              ))}
            </div>
