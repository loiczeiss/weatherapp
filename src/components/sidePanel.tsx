...
function IconSelection(code: number): StaticImageData {
    return iconMap[code] || thunderIcon;
}
...
export default function SidePanel({weatherData}: WeatherDataProps) {
    ...
    const [iconData, setIconData] = useState<StaticImageData[]>([]);
    ...
    useEffect(() => {
        const weatherCodes: number[] = weatherData.daily.weatherCode.slice(0, days);
        const icons: StaticImageData[] = weatherCodes.map((code) => IconSelection(code));
        setIconData(icons);
        ...
        <>
            <Card className="flex flex-col md:w-5/12 lg:w-3/12 items-center h-screen md:h-full" isBlurred>
                ...
                <div className="flex flex-col w-10/12 items-center my-4 md:my-0 lg:my-4">
                    ...
                    <div className="h-48 pb-8 overflow-y-auto overscroll-y-auto scrollbar-hidden">
                        {weatherData.daily.time.slice(0, days).map((time, index) => {
                            const code = weatherData.daily.weatherCode[index];
                            const minTemp = weatherData.daily.temperature2mMin[index];
                            const maxTemp = weatherData.daily.temperature2mMax[index];
                            ...
                            return (
                                ...
                                >
                                    ...
                                    <Skeleton isLoaded={isLoading} className="flex flex-col w-28 rounded-lg">
                                        <p className="text-xs text-white/50 pl-2">
                                            {WeatherDescriptions[code]}
                                        </p>
                                    </Skeleton>
                                    <Skeleton isLoaded={isLoading}
                                        className="flex flex-col sm:ml-6 md:ml-2 lg:ml-4 border-l border-l-white/25 px-2 w-16 rounded-lg">
