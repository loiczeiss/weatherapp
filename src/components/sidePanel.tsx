...
const [days, setDays] = useState(5)
    ...
    <Card className="flex flex-col md:w-5/12 lg:w-3/12 items-center h-screen md:h-full " isBlurred>
      ...
      <div className="flex flex-col w-10/12 items-center my-4 md:my-0 lg:my-4 ">
        ...
        <div className="flex lg:flex-row justify-between mb-4 md:mb-0 lg:mb-4 mt-6 md:mt-0 lg:mt-4">
          <Button className="bg-transparent text-white" onClick={()=>handleButtonsPress(5)}>5 days</Button>
          <Button className="bg-transparent text-white" onClick={()=>handleButtonsPress(10)}>10 days</Button>
          <Button className="bg-transparent text-white" onClick={()=>handleButtonsPress(15)}>15 days</Button>
        </div>
        <div className="h-2/3 overflow-y-auto overscroll-y-auto scrollbar-hidden"> {/* Set height for scrolling */}
  {MockData.daily.time.slice(0, days).map((time: string, index: number) => {
    ...
    return ( // Ensure to return the JSX from the map function
      <div key={index} className="flex flex-row mt-2 w-48 items-center">
